...
struct MaxPoolTaskParams
          ...
          for (int kW = 0; kW < kWp; kW++) {
            for (int c = 0; c < cp; c++) {
              for (int m = 0; m < mp; m++) {
                ...
                layer_p_index = (h * wp * mp) + (w * mp) + m;
                int layer_p_previous_index = ((h * 1 + kH) * (prev_wp * cp) + (w * 1 + kW)) * cp + c;
  ...
  void BatchLayer3DebugTask(void *params) {
        ...
        for (int w = 0; w < wp; w++) {
          ...
          for (int kH = 0; kH < kHp; kH++) {
            for (int kW = 0; kW < kWp; kW++) {
              for (int c = 0; c < cp; c++) {
                p_output_index = (h * wp * cp) + (w * cp) + c;
                layer_p_output[p_output_index] = std::max(layer_4_output[h * 2 + kH][w * 2 + kW][c], layer_p_output[p_output_index]);
              }
            }
          }
        }
        ...
        for (int w = 0; w < wp; w++) {
          ...
          for (int kH = 0; kH < kHp; kH++) {
            for (int kW = 0; kW < kWp; kW++) {
              for (int c = 0; c < cp; c++) {
                p_output_index = (h * wp * cp) + (w * cp) + c;
                layer_p_output[p_output_index] = std::max(layer_8_output[h * 2 + kH][w * 2 + kW][c], layer_p_output[p_output_index]);
              }
            }
          }
        }
...
void predict_SmallCnnActionBINARY5(double const * const x, double * pred) {
    ...
    ConvTaskParams layer2params{26, 26, 12, 3, 3, 1, 28, &layer_2_output[0][0][0], &layer_1_output[0][0][0], const_cast<double *>(layer_2_bias), &layer_2_weight[0][0][0][0]};
    ...
    LogSoftmaxTaskParams layer15params{10, layer_15_output, layer_14_output, pred};
    xSemaphoreGive(mutex);
    xSemaphoreTake(mutex, portMAX_DELAY);
    xTaskCreate(ConvTask, "Layer2", 4096, &layer2params, 1, &layerTaskHandle);
    vTaskDelay(1);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(BatchLayer3DebugTask, "Layer3", 8192, &layer3params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(Step3DTask, "Layer4", 4096, &layer4params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(MaxPoolTask, "Layer5", 4096, &layer5params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(ConvTask, "Layer6", 4096, &layer6params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(BatchNormalization3DTask, "Layer7", 4096, &layer7params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(Step3DTask, "Layer8", 4096, &layer8params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(MaxPoolTask, "Layer9", 8192, &layer9params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    ...
    xTaskCreate(GemmTask, "Layer11", 16192, &layer11params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(BatchNormalization1DTask, "Layer12", 8192, &layer12params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(Step1DTask, "Layer13", 8192, &layer13params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(GemmTask, "Layer14", 8192, &layer14params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
    xTaskCreate(LogSoftmaxTask, "Layer15", 8192, &layer15params, 1, &layerTaskHandle);
    xSemaphoreTake(mutex, 50);
  }
} 
